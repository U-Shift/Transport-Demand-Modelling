...
> **Note:** The Kaplan–Meier method assumes that censoring is independent of survival times. If this is false, the Kaplan–Meier method is inappropriate.
...
> **Note:** The Cox proportional-hazards model is semiparametric method that produces estimated hazard ratios (sometimes called rate ratios or risk ratios).
...
> **Note:** It includes an interaction between the covariate and a function of time (or distance). Log time is often used but it could be any function. If significant then the assumption is violated.
...
```{r}
ggsurvplot(survfit(result.cox), color = "#2E9FDF", ggtheme = theme_minimal())
```
